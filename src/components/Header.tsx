          ...
          {isMenuOpen && (
            <>
              ...
              <motion.div
                ...
                className="md:hidden absolute top-full left-0 right-0 bg-white shadow-2xl border-t border-gray-100 z-50 mx-4 mt-2 rounded-2xl overflow-hidden"
              >
                <div className="px-6 py-6 space-y-1">
                  {navItems.map((item, index) => (
                    <motion.button
                      ...
                      onClick={() => scrollToSection(item.href)}
                      className="block w-full text-left text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200 font-medium py-3 px-4 rounded-xl"
                    >
                      {item.label}
                    </motion.button>
                  ))}
